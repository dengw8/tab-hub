...
新的 `Tab Stash` 功能用于临时维护一组还不想进入书签系统的链接。它和 Dashboard 是平行功能：Dashboard 负责“看见和整理当前打开的 tabs”，Tab Stash 负责“把临时要回来的链接按少量主题分组”。

为了避免功能变成另一套重型 bookmarks，Tab Stash 不支持无限层级。它只保留两层业务结构：root 下是目录，目录下是 tab。

本设计采用单插件架构：Dashboard 仍是固定 new tab 入口，不能关闭；Tab Stash 是默认开启、可关闭的可选 feature。

## 目标

- 在当前 extension 内新增一棵唯一的 Tab Stash。
...
- 支持右键菜单“Add current page to Tab Stash”，并在原生二级菜单中选择已有目录直接保存。
- 为跨设备同步和更多 feature 预留清晰边界。
- 重构代码组织，让 Dashboard、Tab Stash 和未来 feature 横向扩展时不会继续堆进当前单个大脚本。
...
打开新 tab 时永远展示 Dashboard。Tab Stash 默认开启，并在 Dashboard 的 app shell 导航中出现入口。用户可以在设置中关闭 Tab Stash；关闭后隐藏 Tab Stash 入口和未来相关右键菜单，但保留已有 stash 数据。

如果用户已经在 extension 内切到 `#tab-tree` 并刷新页面，可以保留在 Tab Stash 视图；但新开 tab 不携带 hash，仍进入 Dashboard。

点击浏览器扩展栏里的 Tab Out 图标时，展示 toolbar popup。Popup 是后续 feature 控制入口：Dashboard 显示为固定开启；Tab Stash 提供开关；当 Tab Stash 开启时，提供 `Open Tab Stash` 按钮用于打开或聚焦 `index.html#tab-tree`。
...
- `features/tab-tree`：Tab Stash 渲染、编辑、拖拽、搜索和节点操作。
...
Dashboard 和 Tab Stash 不能互相调用内部逻辑，只能通过 shared API 和 storage 协作。
...
如果没有配置，初始化为 `tabTree.enabled = true`。关闭 Tab Stash 时不删除数据。
...
Tab Stash 只有一份 stash，root 固定存在。Root 是不可见的顶层容器，不允许删除、移动、重命名或直接保存 tab。

采用 `nodes` map + `children` 数组，而不是深层嵌套结构，方便移动节点、删除分组数据和未来同步冲突处理。
...
Tab Stash 是完整视图，不作为 Dashboard 右栏。
...
- Tab Stash 默认开启，并能从导航进入。
- 关闭 Tab Stash 后入口隐藏，Dashboard 正常。
- 添加目录。
- 添加 `http/https` tab，并确认标题获取和 fallback。
- 添加 `chrome://`, `about:`, `file:` 类型 URL。
- 点击 tab 节点打开新 tab。
- 编辑、删除、拖拽移动节点。
- 右键当前网页，选择已有目录后加入 Tab Stash。
- 右键当前网页，通过选择器新建目录并立即加入 Tab Stash。
- 搜索时展开命中路径，清空后恢复展开状态。
- 刷新后stash 结构和展开状态保留。
...
4. 实现 feature registry 和 Tab Stash 开关。
5. 实现 Tab Stash 数据操作纯函数，并强制两层结构约束。
6. 实现 Tab Stash UI、modal 和受限拖拽。
7. 实现后台标题获取。
